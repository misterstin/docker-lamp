<?php
require_once('/entity/usuario_class.php');
require_once('/entity/tarea_class.php');
require_once('/entity/fichero_class.php');
function conectaPDO()
{
...
function listaUsuarios()
{
    try {
        ...
        foreach ($resultados as $itinerar){
            $user = new usuario ();
            $user-> setId($itinerar['id']);
            $user-> setUsername($itinerar['username']);
            $user-> setNombre($itinerar['nombre']);
            $user-> setApellidos($itinerar['apellidos']);
            $user-> setContrasena($itinerar['contrasena']);
            $user-> setRol($itinerar['rol']);
...
function nuevoUsuario($usuario)
{
    try{
        $con = conectaPDO();
        $stmt = $con->prepare("INSERT INTO usuarios (nombre, apellidos, username, rol, 
        contrasena) VALUES (:nombre, :apellidos, :username, :rol, :contrasena)");
